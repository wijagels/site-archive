...
void TcpTransport::connect(std::string_view addr, std::string_view service, error_callback on_connect) {
  return m_resolver.async_resolve(addr, service,
                                  [=, on_connect = std::move(on_connect)](boost::system::error_code ec,
                                                                          const tcp::resolver::results_type &results) {
                                    return on_resolve(ec, results, std::move(on_connect));
                                  });
}

void TcpTransport::connect(std::string_view addr, std::string_view service, boost::asio::yield_context yield) {
...
void TcpTransport::on_resolve(boost::system::error_code ec, const tcp::resolver::results_type &results,
  ...
  if (!ec)
    return asio::async_connect(m_socket, results,
                               [=, on_connect = std::move(on_connect)](auto connect_ec, auto /*endpoint*/) {
